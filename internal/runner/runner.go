...
// Run executes the spec, streaming stdout/stderr to the parent process while
// also capturing the combined output (returned as a string, so callers can tell
// whether an agent "talked" without editing files). The context cancels the
// child (Ctrl-C). A non-zero exit is returned as a Result, not an error — only a
// failure to start (or a timeout) is an error.
func Run(ctx context.Context, spec Spec) (string, Result, error) {
	if spec.Timeout > 0 {
...
// RunCaptureQuiet runs the spec capturing stdout, discarding stderr entirely —
// so an agent's log/progress noise never reaches the terminal. Used by the
// router for classification/answers (including inside the TUI, where any stray
// output would corrupt the render).
func RunCaptureQuiet(ctx context.Context, spec Spec) (string, Result, error) {
	if spec.Timeout > 0 {
		var cancel context.CancelFunc
		ctx, cancel = context.WithTimeout(ctx, spec.Timeout)
		defer cancel()
